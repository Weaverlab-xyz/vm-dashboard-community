...
# The external ID is generated by Workload Credentials and read back, not supplied.
#
# Worth spelling out because the sources disagree. The provider's PUBLISHED DOCS list
# external_id as a required input; the shipped provider (v1.3.1) marks it read-only and
# `terraform validate` rejects setting it. The binary is right and the docs are stale. An earlier revision of this
# module supplied a random_uuid and failed validation.
#
# That is also why role_arn below is a CONSTRUCTED STRING rather than a reference to
# aws_iam_role.integration. The role's trust policy needs the external id, so the
# integration has to exist first; referencing the role here as well would be a cycle.
# The role name is known in advance, so the ARN can be built without the dependency.
...
locals {
  integration_role_arn = var.enable_aws ? format(
    "arn:aws:iam::%s:role/beyondtrust/%s",
    one(data.aws_caller_identity.current[*].account_id),
    var.aws_integration_role_name,
  ) : ""
}
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        ...
        StringEquals = {
          "sts:ExternalId" = beyondtrust_workload_credentials_aws_integration.this[0].external_id
        }
      }
    }]
  })
...
resource "beyondtrust_workload_credentials_aws_integration" "this" {
  ...
  role_arn = local.integration_role_arn

  # Deliberately no depends_on. This is created FIRST, before the role it names exists,
  # because the role's trust policy needs the external id this resource generates. That
  # is the shape BeyondTrust's own Terraform example uses, so presumably the service does
  # not verify the role at registration time — but it is the one ordering assumption in
  # this module that only a real apply can confirm.
}
