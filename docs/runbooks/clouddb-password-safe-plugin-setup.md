...
| `SA:` | Broker with no GCP identity at all — and every Cloud SQL **SQL Server** onboarding, which cannot use `reference` mode | The base64 of the rotator's JSON key in password segment 1. In `create` mode the dashboard **mints one per database** for `clouddb_ps_gcp_rotator_service_account` and deletes it at teardown — it needs `roles/iam.serviceAccountKeyAdmin` on that account, and `clouddb_ps_gcp_sa_key` overrides it where that is out of reach or `constraints/iam.disableServiceAccountKeyCreation` is set. In `reference` mode you paste it yourself. The ~3.2 KB composite stores fine, but the plugin refuses a **write-back** over 1000 characters — so leave password management off for that account |
...
| (Azure) `Thread was interrupted from a waiting state`, ~30s after the attempt started | the Azure Run Command plugins read the managed system's Timeout as **seconds** (like GCP, unlike AWS), and until 2026-09-02 the dashboard registered **no** Timeout at all — so the field sat at Password Safe's default of `30` and the plugin's watchdog interrupted its own wait 30s in. One Azure Run Command round trip is routinely 20-60s before `psql` even runs, and the plugin answers a `409` by retrying 5 times at 15s, so the wait has to cover that ladder too. New Azure DB managed systems are registered with Timeout `180`; a system onboarded before that carries `30` — re-register it from the row's **Register in Password Safe** action, or raise Timeout to `180` in BeyondInsight. As on GCP, a diagnostic printing `timeout 30000 msec` is that `30` after the plugin's seconds-to-milliseconds conversion, **not** evidence that anything registered `30000` |
...
| Azure Run Command permission denied (`AuthorizationFailed` on `virtualMachines/runCommand/action`) | the identity the plugin **resolved** lacks Virtual Machine Contributor on the **jump-VM** resource group — and that is not always the SP you granted it to. The 403 names the client id and object id it used: check them against the RG's assignments before granting anything (`az role assignment list --scope <rg> --include-inherited`, filtering on `principalId`, because a Graph lookup of a sandbox-tenant SP fails for a corp guest identity). A functional-account name without its `SP:` prefix skips tier 1 entirely, so the id in the error is then the broker's `ControlPlane`/ambient identity rather than the FA's own SP — granting *that* one the role papers over an FA whose credentials are never used (§0, §1.3) |
