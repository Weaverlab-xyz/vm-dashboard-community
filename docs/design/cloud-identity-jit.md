...
- `duration` (number) — the requested TTL. Bounded by the workflow's
  `max_duration` (our `machine_ttl_ceiling`).
- `justification` (string) — auto-generated by the submitter; we
  include `operation`, `payload_hash`, and the dashboard's
  `requester_user_id`.
- `target` (object) — either an `AccessRequestBundleTargetCreateSchema`
  (bundle id) or `AccessRequestRoleTargetCreateSchema` (role id). The
  operation matrix in §6.3 stores the Entitle bundle/role id for each
  operation key.
- `behalfOf` (object, **optional but critical for us**) — submits the
  request *on behalf of* another identity. This is how we encode the
  machine principal: the dashboard authenticates with its own bearer
  token, then submits each elevation request `behalfOf` the synthetic
...
**This pattern is natively supported by Entitle.** The
`POST /v1/accessRequests` endpoint accepts a `behalfOf` field that
*"enables requests on behalf of another user/service account"* (per
the API reference). The dashboard authenticates to Entitle with its
own bearer token, and each request specifies `behalfOf` pointing at
the synthetic cloud-principal user. The workflow then evaluates the
rule against the `behalfOf` identity — not the API caller — so the
auto-approve rule fires only for these synthetic users, never for a
human admin's token reused against the same endpoint.
