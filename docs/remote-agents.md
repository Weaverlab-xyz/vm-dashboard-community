...
| `'<op>' is not available on an agent-bound <kind> connection` (501) | Working as intended, and not a policy or grant problem — this build has no verb that performs that operation on that product, and it refuses rather than sending the nearest one. Suspend, Resume, Pause and Save are the usual ones. See [The verbs](remote-agents/hypervisors.md#the-verbs) for which button works on which product; graceful Shutdown and Reboot are **not** in this group any more. |
...
| `the Docker socket … is mounted but this process cannot open it` | **Permissions, not the mount.** The socket is usually `srw-rw---- root docker` and the agent runs as a non-root uid with no supplementary groups, so it gets `EACCES` on a socket that is present. Add the socket's group to the container — `--group-add <gid>`, or `group_add:` in Compose — or mount a rootless socket the agent's user owns. Re-applying the overlay changes nothing |
...
| A VM does not appear in the picker at all | Either its connection is not agent-bound, or the row is untagged and you are not an admin — an untagged synced VM is admin-only until an admin assigns a workgroup, the same rule every hypervisor page keeps. A Config-Management route does **not** make a VM appear: the address rules in the two rows above still decide that. |
...
| `Agent 'x' is not granted the Config-Management job type` | The dashboard operator's half of the permission. Agents page → that agent → grant `agent_ansible`. Your `policy.yaml` still has to grant it too. **The name in that message is the agent that would EXECUTE**, which is not always the one under Connections — a [Config-Management route](remote-agents/config-runs.md#which-agent-executes-a-run) may have sent this address elsewhere. Grant it to the agent the message names. |
| The picker says a VM runs on an agent that did not discover it | Working as intended: a Config-Management route sends that address range to that agent. The line under the picker names the route that decided. Clear **Active** on it to go back to the discovering agent. |
| `This VM's address … is routed to agent 'x' … so agent 'y' cannot run against it` | A stale browser tab holding a target list from before the route existed. Reload the Config Management page — the target list is what computes the executor, and the server re-derives it on every run. |
| `policy.yaml does not allow Config Management against x:22`, from an agent that has never appeared in a job | **The most common new failure once routes are in use.** A routed agent has its own `policy.yaml`, and its `ansible.targets:` must cover the range — the discovering agent's file grants nothing for a run it is not executing. Two agents, two files, and only one of them was edited. The route form on Remote Agents → Config Routes renders the exact block to paste. |
| `this agent's policy.yaml does not enable Config Management`, on an agent that discovers nothing | Same cause as the row above, one step earlier: the routed agent has no `ansible:` block at all. Add it there, not on the discovering agent. |
| A run is refused as *not online*, but the agent you were watching is online | You were watching the **discovering** agent. The executing agent's status is what gates the enqueue, and with a route in play that is a different row on the Agents tab. |
| `… is already routed to agent 'x'` when adding a route | One address range may name only one executing agent, so that the answer to "which agent runs this" never depends on row order. Edit the existing route, or add a **narrower** range — the most specific range wins. |
| Deleted a route and runs went back to the old agent | Expected. An absent route means "the agent that discovered the target", which is a correct answer rather than an empty one, so nothing is promoted in its place. Clear **Active** instead if you want the record of the decision kept. |
