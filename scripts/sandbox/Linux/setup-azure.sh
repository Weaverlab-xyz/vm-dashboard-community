...
# …), so changing only AZURE_LOCATION does not describe a second region — it
# describes the FIRST one, in the wrong place. Azure agrees and rejects it, but its
# message is `InvalidResourceGroupLocation: Invalid resource group location
# 'westus2'. The Resource group already exists in location 'centralus'`, which names
# neither this script, nor SANDBOX_NAME_PREFIX, nor what to do about it. Say it here
# instead, before the first `az` write rather than during it.
...
if [[ -n "$EXISTING_RG_LOCATION" ]] \
  ...
  die "Resource group $RG already exists in $EXISTING_RG_LOCATION, but AZURE_LOCATION is $LOCATION.
     ...
     azure_region.$LOCATION.*. Do NOT hand-copy $EXISTING_RG_LOCATION's subnet ids into
...
# network_mode=vpc Cloud Functions integrate with the VNet through THIS subnet. It
# has to be its own: a delegated subnet hosts exactly one service, so it can share
# neither db-subnet (DBforPostgreSQL) nor aci-subnet (ContainerInstance).
#
...
# own calls to AzureWebJobsStorage and WEBSITE_RUN_FROM_PACKAGE. Attaching $NSG
# (which denies the Internet service tag outbound) would stop the host reaching
# them and the app would boot to zero indexed functions. Same reason
# jumpoint-subnet and db-sqlserver-subnet have none.
#
...
# @Microsoft.KeyVault(SecretUri=...) — which does traverse the VNet once
# vnet_route_all_enabled is on) on the Azure backbone, without depending on the
# subnet's implicit default outbound access. Generic internet egress from a
# vnet-mode function still needs a NAT gateway (~$32/mo) — not created here.
FN_SUBNET="functions-subnet"
az network vnet subnet create -g "$RG" --vnet-name "$VNET" -n "$FN_SUBNET" \
  --address-prefix 10.99.9.0/24 \
  --delegations Microsoft.Web/serverFarms \
  --service-endpoints Microsoft.Storage Microsoft.KeyVault >/dev/null
ok "Functions subnet $FN_SUBNET (10.99.9.0/24, delegated to Microsoft.Web/serverFarms, service endpoints Storage+KeyVault)"
FN_SUBNET_ID="$(az network vnet subnet show -g "$RG" --vnet-name "$VNET" -n "$FN_SUBNET" --query id -o tsv)"
state_write azure functions_subnet_id "$FN_SUBNET_ID"
...
if az role assignment list --assignee "$SP_OBJECT_ID" --scope "$SUB_SCOPE" \
     --role "Cost Management Reader" --query '[0].id' -o tsv 2>/dev/null | grep -q .; then
  ok "SP already has Cost Management Reader on the subscription"
elif retry 8 5 az role assignment create --assignee-object-id "$SP_OBJECT_ID" \
       --assignee-principal-type ServicePrincipal \
       --role "Cost Management Reader" --scope "$SUB_SCOPE" >/dev/null 2>&1; then
  ok "Granted SP Cost Management Reader on the subscription (Cloud Costs page)"
else
  warn "Could not grant Cost Management Reader on the subscription — the Cloud Costs"
  warn "  page will show Azure as 'unavailable' (401). Grant it with an account that has"
  warn "  Owner/User Access Administrator on the subscription:"
  warn "    az role assignment create --assignee $SP_APP_ID \\"
  warn "      --role 'Cost Management Reader' --scope $SUB_SCOPE"
fi
...
if [[ -z "${SANDBOX_SKIP_ACR:-}" ]]; then
  ...
  ACR_PROV_STATE="$(az provider show --namespace Microsoft.ContainerRegistry \
    --query registrationState -o tsv 2>/dev/null || echo NotRegistered)"
  if [[ "$ACR_PROV_STATE" != "Registered" ]]; then
    az provider register --namespace Microsoft.ContainerRegistry --wait >/dev/null
    ok "Registered Microsoft.ContainerRegistry provider"
  fi
  ...
  ACR_NAME="$(printf '%sacr%s' "${NAME//-/}" "$(printf '%s' "$SUBSCRIPTION_ID" | tr -d '-' | head -c8)" \
    | tr 'A-Z' 'a-z' | head -c50)"
  if az acr show -g "$RG" -n "$ACR_NAME" >/dev/null 2>&1; then
    ok "Reusing ACR $ACR_NAME"
  else
    az acr create -g "$RG" -n "$ACR_NAME" -l "$LOCATION" --sku Basic --tags "$TAGS" >/dev/null
    ok "Created ACR $ACR_NAME (Basic SKU)"
  fi
  ACR_LOGIN_SERVER="$(az acr show -g "$RG" -n "$ACR_NAME" --query loginServer -o tsv)"
  ACR_ID="$(az acr show -g "$RG" -n "$ACR_NAME" --query id -o tsv)"

  # Grant the SP AcrPull so the dashboard's ACI runners can pull. Idempotent; retry absorbs
  # the ARM PrincipalNotFound race for a freshly created SP (same pattern as §6b).
  if az role assignment list --assignee "$SP_OBJECT_ID" --scope "$ACR_ID" \
       --role AcrPull --query '[0].id' -o tsv 2>/dev/null | grep -q .; then
    ok "SP already has AcrPull on $ACR_NAME"
  else
    retry 8 5 az role assignment create --assignee-object-id "$SP_OBJECT_ID" \
      --assignee-principal-type ServicePrincipal \
      --role AcrPull --scope "$ACR_ID" >/dev/null
    ok "Granted SP AcrPull on $ACR_NAME"
  fi
  ...
  for _img in \
    ...
    if [[ -n "${DOCKERHUB_USERNAME:-}" && -n "${DOCKERHUB_TOKEN:-}" ]]; then
      ...
      retry 5 10 az acr import -n "$ACR_NAME" --source "docker.io/$_img" --image "$_img" --force >/dev/null
    fi
    ok "Mirrored $_img → $ACR_LOGIN_SERVER/$_img"
  done

  state_write azure acr_name         "$ACR_NAME"
  state_write azure acr_id           "$ACR_ID"
  state_write azure acr_login_server "$ACR_LOGIN_SERVER"
else
  ok "Skipping ACR (SANDBOX_SKIP_ACR set) — ACI runners will pull from Docker Hub"
fi
...
PROMOTE_IMAGE="${ACR_LOGIN_SERVER:+$ACR_LOGIN_SERVER/}chrweav/dashboard-promote-runner:latest"
...
_cfg=(
  ...
  "secrets_azure_kv_url=$KV_URL                            # Same vault — the key the Cloud Functions path reads"
  ...
  "function_package_azure_container=function-packages      # Blob container on $SA_NAME (the app creates it)"
  "azure_functions_plan_sku=B1                             # Cheapest SKU doing BOTH VNet integration and run-from-package (Y1 cannot)"
  "azure_functions_subnet_id=$FN_SUBNET_ID  # network_mode=vpc: delegated to Microsoft.Web/serverFarms"
  ""
  ...
  "azure_region.$LOCATION.db_mysql_private_dns_zone_id=$DB_MYSQL_DNS_ZONE_ID"
  ...
  "promote_runner_image=$PROMOTE_IMAGE   # ACR mirror when present (else public Docker Hub image)"
...
# azure_acr_server); ansible takes the full ACR path (it does not prepend). These are flat
# global keys (one ACR serves all regions). Skipped when SANDBOX_SKIP_ACR was set.
