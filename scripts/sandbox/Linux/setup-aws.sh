...
JUMPOINT_SG="$(make_sg "${NAME}-jumpoint-sg" "Jumpoint ECS task - outbound to internet only, no ingress")"
...
_cfg=(
  ...
  "# keys land in aws_region_configs, so running this script again with a"
  "# different AWS_REGION MERGES that region in rather than overwriting this one."
  "# (Every field falls back to its flat key when blank, so a single-region"
  "# install behaves exactly as before.)"
  "aws_region.$REGION.default_subnet_id=$PRIVATE_SUBNET_ID"
  "aws_region.$REGION.default_security_group_id=$VM_SG"
  "aws_region.$REGION.ssh_key_secret=$SSH_SECRET_NAME"
  "aws_region.$REGION.db_subnet_group_name=$DB_SUBNET_GROUP_NAME"
  "aws_region.$REGION.db_parameter_group_name=$DB_PARAM_GROUP_NAME"
  "aws_region.$REGION.db_mysql_parameter_group_name=$DB_MYSQL_PARAM_GROUP_NAME"
  "aws_region.$REGION.db_security_group_id=$DB_SG"
  "aws_region.$REGION.vpc_id=$VPC_ID"
  "aws_region.$REGION.vpc_cidr=10.99.0.0/16"
  "aws_region.$REGION.private_route_table_id=$PRIVATE_RT_ID"
  "aws_region.$REGION.nat_security_group_id=$NAT_SG"
  "aws_region.$REGION.ecs_subnet_id=$PUBLIC_SUBNET_ID"
  "aws_region.$REGION.ecs_security_group_ids=$JUMPOINT_SG"
