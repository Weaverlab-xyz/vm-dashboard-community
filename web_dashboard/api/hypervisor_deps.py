...
resolution failure into an HTTP status, and offer that connection to its bound agent
before dialling it directly. Keeping all of it here means the routers cannot drift on
any of them — and every one has drifted at least once when they each kept a copy.

The bulk-power fan-out used to live here too and now lives in ``api/power_batch.py``,
re-exported at the bottom: the cloud routers need it as well, and a module named for
hypervisors is the wrong place for the four of them to import from.
...
# ── Bulk power ────────────────────────────────────────────────────────────────
#
# Moved to `api/power_batch.py`, because the four cloud routers queue power batches too
# and importing them from a hypervisor-named module invites a second copy. Re-exported
# here so the six on-prem routers' imports are unchanged — and so this module still
# reads as the one place a hypervisor router looks.
from .power_batch import (BULK_MAX_TARGETS, queue_power_batch,  # noqa: E402,F401
                          run_power_batch)
