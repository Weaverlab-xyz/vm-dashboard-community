...
async def asset_size(backend: str, name: str) -> int:
    ...
    Read from the LISTING rather than by fetching, which is the whole point: the caller is
    deciding whether the file is too big to move, and answering that by moving it would be
    self-defeating. -1 rather than 0 for "not found": a zero-byte asset is a real answer,
    and a caller that treats "missing" as "small" would embed nothing and fail later with a
    less useful message.
    """
    _validate_backend(backend)
    for item in await _run_op(backend, "list"):
...
def upload_ceiling_reason(backend: str) -> str:
    ...
    """
    if backend == "agent_local":
        # Names the remedy, not just the cause. Rewriting this to drop "use a cloud
        # backend" is what test_an_oversize_upload_is_refused_before_it_is_queued exists to
        # catch: an operator told only that a file is too big goes looking for a setting to
        # raise, and for this backend there is none to find.
        return ("a share reached through an agent moves files inside a signed job "
                "envelope, which is capped at that size, so playbooks and scripts fit "
                "and packages do not — use a cloud backend for anything larger")
