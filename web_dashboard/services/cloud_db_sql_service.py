...
# `-Nm -C` all mean "this connection must be encrypted; do not check the certificate".
# Verifying instead (`verify-full` / `VERIFY_IDENTITY` / `-Nm` without `-C`) would need
...
def _mssql_command(*, host, port, database, admin_user, admin_password, image, statements) -> str:
    ...
    through docker.
    """
    # -b: exit non-zero on SQL error. -Nm (encryption mandatory) -C (trust the
    # server cert without checking it against a CA) — see the TLS note above.
    # Batches joined with GO.
    #
    # The encryption switch's value is ATTACHED, and that is the whole point of
    # this line: `-N[s|m|o]` (strict / mandatory / optional) is a single token.
    # Space-separate it and /opt/mssql-tools18/bin/sqlcmd rejects its own command
    # line before opening a socket — `Sqlcmd: Command -N: Invalid Parameters
    # passed.`, rc=1, observed live on Azure at 25% "Creating the rotatable
    # managed database user…" with `-N t` on 2026-09-02, and AGAIN on 2026-09-03
    # after that was replaced with a bare `-N`. (`t` was wrong twice over —
    # `true|false|disable` is the separate *Go* sqlcmd's vocabulary, not this
    # ODBC one's.)
    #
    # The second failure cannot tell its two candidates apart, because the
    # message names the SWITCH and not the value either way: either that host
    # was still running the un-rebuilt image and sending `-N t`, or -N takes a
    # parameter here and the bare form swallowed the `-C` that followed it.
    # `-Nm` is correct under both readings, and is the one form that cannot be
    # misparsed at all: attached, and `o|m|s` have been its accepted values on
    # Linux since sqlcmd 18.0 — the only major mssql-tools18 ships. It is also
    # deliberately explicit rather than default-dependent: omitting -N
    # altogether means mandatory on current builds but `-No` (optional — i.e.
    # cleartext if the server declines) on older ones, which is the exact
    # downgrade the TLS note above exists to prevent. `-C` waives cert
    # validation.
    batch = "\nGO\n".join(statements) + "\nGO\n"
    args = [
