...
| [Personas](docs/profiles/demo/personas/README.md) | One page per role — Cloud Ops, DevOps, hypervisor admin, IT, OT/ICS, DBA, security analyst, SRE, network/firewall admin, FinOps/cloud governance — covering what that person owns, the four-layer story in their language (provisioning → PRA → Password Safe → Entitle), the use cases to run, and which integrations each needs. The in-app **Use cases** page is the same catalog, with each card reporting whether this instance can actually run it. | You are presenting to a specific role and want the story and the click path, rather than a feature list. |
