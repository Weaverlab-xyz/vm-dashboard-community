...
| [POV Instance](docs/pov-instance.md) | Running a **second** dashboard for customer POV/POC environments: why `install_profile` makes demo and POV mutually exclusive, which features each profile gets, the **BeyondTrust tenant registry** that replaces the singletons (and why an explicit tenant never falls back to the default), the four things that bite — its own JWT key, a cold first boot, a reachable agent endpoint, and an unarmed auto-delete timer — and the **per-POV use-case checklist**, which asks what *this* POV can demonstrate rather than what this instance supports. | You do customer proof-of-value work and don't want it sharing an instance, a database or a BeyondTrust tenant with your demo estate. |
