...
| [Entitle User Identity](docs/integrations/entitle-user-identity.md) | Just-in-time DASHBOARD permissions granted by Entitle over REST, for local and any-OIDC users — replacing the Entra-group indirection, which only worked for Entra and only took effect at next login. | You want time-boxed dashboard access without standing admins. |
